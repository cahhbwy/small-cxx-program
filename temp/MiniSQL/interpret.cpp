...
bool CInterpret::GetWord(CString& src, CString& des)
{
	unsigned int srcpos = 0, despos = 0;
	char temp = ' ';

	for(;srcpos<src.GetLength();srcpos++) {
		if(temp == ' ' || temp == '\t' || temp == 10 || temp == 13)
			temp = src.GetAt(srcpos);
		else break;
	}
	if(srcpos == src.GetLength() && (temp == ' ' || temp == '\t' || temp == 10 || temp == 13))
		return false;
...
bool CInterpret::GetStr(CString& src, CString& des)
{
	unsigned int pos=0;
	char temp;
	if (src.GetAt(0) == '\'') {
	...
	else for(;pos<src.GetLength();pos++) {
		if((temp = src.GetAt(pos)) != '\'')
			des.SetAt(pos,temp);
		else {
			des.SetAt(pos,'\0');
			src.Delete(0,pos);
...
{
	char tempbuf[COMLEN];
	CString word = tempbuf;
	CString temp = command;
	...
	else if(word == "create")
	{
		...
		if(word == "table")
		{
			...
			else if(word == "char")
			{
				...
				if(!flag)
					return;
				if(!IsInt((LPCTSTR)word))
					return;
				tempcol -> collength = atoi(word) + 1;
			...
			while(word == ",")
			{
				...
				if(word == "primary")
				{
					...
					while(tempcol)
					{
						if(!strcmp(word, tempcol -> colname))
						{
							FindPriKey = 1;
							tempcol -> IsPrimary = 1;
							tempcol -> IsUnique = 1;
